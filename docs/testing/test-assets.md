...
| 后端冒烟验证脚本 | `spring-backend/scripts/verify.js` | Node 接口级冒烟，覆盖核心 HTTP 链路 |
| 编译提交延迟实测 | `spring-backend/scripts/measure-submit-latency.mjs` | 量 `POST /api/compile/{id}` 的提交耗时（异步契约 G1），可一键复现 |
| 后端离线评估集 | `spring-backend/eval/` | E1 评估：提示词→chat→编译→静态违例检测 |
| 测试数据 | `docs/testing/testdata/` | 25 个数据文件（CSV+XLSX），对应用例 01–32 |
| 手动测试用例 | `docs/testing/manual-test-cases.md` | 手动测试全集（01–10 基础 + 11–32 进阶） |
| 用例清单（精简版） | `docs/testing/manual-test-cases-checklist.md` | 32 例，17 号放弃 → **有效 31 例**；含明细与记录表 |
| 业务用例批量执行器 | `spring-backend/eval/run-cases.mjs` | 跑上面 32 例（`--compile --rag=on/off --tag=…`），产物落 `spring-backend/eval/results/`（历史轮次产物在 `spring-backend/test/results/`，均已 gitignore） |
| RAG on/off 实跑对比 | `docs/testing/eval-rag-onoff-comparison-32cases.md` | 31 例 × 2 轮（2026-09-16）：逐例数字、违例分布、62 张目视评分 |
...
位置：`spring-backend/scripts/verify.js`（Node，零第三方依赖）
...
```bash
node scripts/verify.js               # 自动启动后端并全量验证
node scripts/verify.js --no-start    # 后端已在运行时只做接口验证
```
...
```bash
node scripts/measure-submit-latency.mjs --email=<user@example.com> --password=<pwd> --count=4 --rounds=3
...
位置：`docs/testing/testdata/`，25 个文件，对应用例编号：
...
| 文档 | 说明 |
|------|------|
| `docs/testing/manual-test-cases.md` | 手动测试用例全集（01–10 基础 + §9 进阶 11–32），含前置准备、通用流程、预期结果、实测结论 |
| `docs/testing/manual-test-cases-checklist.md` | 精简版清单（32 例 / 17 号放弃 → 有效 31），供 `run-cases.mjs` 批量回归 |
| `docs/testing/eval-rag-onoff-comparison-32cases.md` | RAG on/off 31 例实跑对比（2026-09-16）与目视评分 |
...
| `docs/plans/plan-AI-落地清单.md` | AI 能力落地清单 |

## 7. 前端测试说明

前端（Vue 3）当前**无自动化测试**：`package.json` 仅含 `serve / build / lint` 脚本；已引入 `playwright` 依赖但尚无测试文件或配置。前端的质量保障依赖 `docs/testing/manual-test-cases.md` 手动回归。
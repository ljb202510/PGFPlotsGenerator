...
- 后端启动需在 `spring-backend` 目录（`spring.config.import: optional:file:../.env[.properties]`）。
...
`spring-backend/scripts/verify.js` 第 2 段，紧跟现有 `responseTime` 断言（2026-09-14 快照约 L168-172）之后追加两条：
...
| T7 | verify.js 断言 +2 | `spring-backend/scripts/verify.js` | T3、T4 |

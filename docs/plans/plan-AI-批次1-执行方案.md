...
| 4 | 环境变量存在 | 检查 `../.env` 或系统环境变量 | `DEEPSEEK_API_KEY` 或 `NSCC_API_KEY` 至少一个非空 |

> 注意：`application.yml` 通过 `spring.config.import: optional:file:../.env[.properties]` 共享环境文件，后端必须在 `spring-backend` 目录下启动；`verify.js` 已自动处理。

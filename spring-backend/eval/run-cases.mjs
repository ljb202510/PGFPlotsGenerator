#!/usr/bin/env node
/**
 * 自动化回归脚本：把 docs/testing/manual-test-cases-checklist.md 里每个用例的
 ...
 *     EMAIL=x@x.com PASSWORD=xxx node eval/run-cases.mjs
 *     node eval/run-cases.mjs --email=x@x.com --password=xxx --base=http://localhost:3000
 *     node eval/run-cases.mjs --model=siliconflow --only=25,27,32
 *     node eval/run-cases.mjs --compile            # 生成后额外提交 XeLaTeX 编译并轮询 PDF 状态
 *     node eval/run-cases.mjs --no-upload          # 跳过「先上传、用上传得到的 data_id」的整条链路（仅测试用）
 *     node eval/run-cases.mjs --rag=on --tag=rag-on-32cases   # 仅给本轮打 RAG 开关标记（真正生效靠进程环境变量 RAG_ENABLED）
 *
 * 产出：控制台汇总 + eval/results/<tag>.json。
 */
...
const TESTDATA = path.join(__dirname, '..', '..', 'docs', 'testing', 'testdata');
